...
/**             Problem statement
 ...
 and return the lexicographically smallest result that can be obtained by placing the symbols of s2 between the symbols of s1 in such a way
 that maintains the relative order of the characters in each string.
 ...
 If the number of occurrences are equal, then the characters should be compared in the usual lexicographical way.
 If both number of occurences and characters are equal, you should take the characters from the first string to the result.
...
public class Task3 {
    ...
    public static void main(String[] args) {
        ...
        //String s1 = "super", s2 = "tower";  // "stouperwer"
        String s1 = "dce" , s2 = "cccbd";       // dcecccbd
        System.out.println(task3.merge(s1, s2));
    }
    ...
    String merge(String s1, String s2) {
        ...
        while ((counter1 != s1.length()) || (counter2 != s2.length())) {
            ...
            if(counter1 == s1.length()) {
                builder.append(s2.charAt(counter2));
                counter2++;
            }
            else if(counter2 == s2.length()) {
                builder.append(s1.charAt(counter1));
                counter1++;
            }
            else if(s1.charAt(counter1) <= s2.charAt(counter2)) {
                builder.append(s1.charAt(counter1));
                counter1++;
            } else {
                builder.append(s2.charAt(counter2));
